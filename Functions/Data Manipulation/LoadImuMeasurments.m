function [imuData, frameRange] = LoadImuMeasurments(imuDataFolder, imageTimestamps)
% loadImageData Read captured image data into memory. Tries to load
% individual images or a saved mat file if one exists.
    

    %Read IMU data
    oxtsData = LoadOxts(imuDataFolder,imageTimestamps);
    % load IMU Data
    ...
    for meas_i=1:length(frameRange)
     ...
     imuData.measOmega(:,meas_i) = oxtsData{i}(omega_index);
      ...
      imuData.measOrient(:,meas_i) = RotationMatrix2Quaterion(R);
      imuData.measVel(:,meas_i) =  oxtsData{i}(v_index);
      imuData.measAccel(:,meas_i) =  oxtsData{i}(a_index)' - R'*[0;0;9.81];

     
      if meas_i == 1
          imuData.initialVelocity = RotationOfBody(oxtsData{i})'*[ oxtsData{i}(8); oxtsData{i}(7); 0; ];
      end
    end
